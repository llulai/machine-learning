...
**Answer: ** I think that the values of these variables will affect the price of a house in different ways, not only the direction but also the proportion. Withough doing a regression first, I can only make an educated guess for the direction.
* 'RM': I think it will have a positive impact in the price of the house, the more rooms the higher the price.
* 'LSTAT': I think it will have a negative impact, the higher the percentage the lower the price of the house.
* 'PTRATIO': I think it will have a negative impact, the more teachers available per pupil, the higher the price of the house.
...
**Answer: ** We reduce the possibility of overfitting the model. By keeping some proportion of data we have a different dataset to test our model, check if the predictions are reliable and be more confident about the value that R^2 coefficient is giving us.
...
**Answer: ** The scores of grap with max_dept = 3 seems to have converged to determined values. So in this case it seems that adding more training poings wont benefit the fit of the model. When we have very few training points the model seems to be overffited, this because the training score is really high, while the testing score is really low.
...
**Answer: ** Between 4 and 6, as we have more simple models, the scores (training and validation) get much worse, so the model is not capable of giving good predictions. While if we make more complex models, we add too much variance to it.
